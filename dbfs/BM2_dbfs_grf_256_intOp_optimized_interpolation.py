...
# data generator for grf to access coarsening function
from ..dataprep.generate_grf_scalar_spectral import RandomFieldGenerator2D
...
def coarsen_field(
    ...
    coarse = th.nn.functional.interpolate(
        smooth, scale_factor=scale_factor, mode=method
    )
    ...
    for iteration in range(start_iteration + 1, iterations + 1):
        ...
        for step in range(step + 1, step + training_steps + 1):
            ...
            with th.autocast(device_type="cuda", dtype=th.float16, enabled=True):
                with torch.profiler.record_function("BM2_Training_ForwardBackward"):
                    ...
                    if intOp_scale_factor > 0:
                        ...
                        losses_intOperator = (
                            target_b_t - coarsen_field(b_1, H=H, downsample_factor=1)
                        ) ** 2
            ...
            if step % (test_steps) == 0 and rank == 0:
                ...
                run_name = wandb.run.name
                save_checkpoint(
                    saves, step, console, run_name, checkpoint_dir, checkpoint_prefix
                )
