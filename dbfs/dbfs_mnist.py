            ...
            if step % test_steps == 0:
                ...
                with th.no_grad():
                    ...
                    for te_x_0, te_x_1 in zip(te_loader_x_0, te_loader_x_1):
                        ...
                        te_x_0 = F.resize(te_x_0[0], 64).to(device)
                        te_x_1 = F.resize(te_x_1[0], 64).to(device)
                    ...
                    for i, ti in enumerate(
                        ...
                        if rank == 0: wandb.log(
                            {f"{direction}/test_64/x[{i}-{5}]": image_grid(te_s_path[ti])},
                            step=step,
                        )
                    for i, ti in enumerate(resample_indices(discretization_steps, 5)):
                        if rank == 0: wandb.log(
                            {f"{direction}/test_64/p[{i}-{5}]": image_grid(te_p_path[ti])},
                            step=step,
                        )
...
def restore_checkpoint(saves, console):
    # Simplified from https://github.com/ghliu/SB-FBSDE/blob/main/util.py:
    i = 0
    load_name = './checkpoint/mnist.npz'
