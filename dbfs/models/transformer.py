...
class QuerieEmbedder(nn.Module):
    ...
    def forward(self, grid, c):
        ...
        queries = grid + gate_mlp.unsqueeze(1) * self.mlp(modulate(self.norm1(grid), shift_mlp, scale_mlp))
        return queries
...
class CrossAttnBlock(nn.Module):
    ...
    def forward(self, queries, context, c):
        shift_msa_q, scale_msa_q, shift_msa_c, scale_msa_c, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(c).chunk(8, dim=1)
        queries = queries + gate_msa.unsqueeze(1) * self.attn(modulate(self.norm1(queries), shift_msa_q, scale_msa_q), 
                                                              modulate(self.norm2(context), shift_msa_c, scale_msa_c))
        queries = queries + gate_mlp.unsqueeze(1) * self.mlp(modulate(self.norm3(queries), shift_mlp, scale_mlp))
        return queries
    
    

class CrossAttention(nn.Module):
    def __init__(self, query_dim, context_dim = None, num_heads = 8, dropout = 0.,):
        super().__init__()
...
class OperatorTransformer(nn.Module):
    def __init__(self,
        ...
        for _ in range(depth_enc):
            enc_attns = nn.ModuleList([AttnBlock(latent_dim, num_heads) for _ in range(self_per_cross_attn)])
            self.enc_block.append(nn.ModuleList([CrossAttnBlock(latent_dim, cross_heads), enc_attns]))

        self.dec_block = nn.ModuleList([])
        for _ in range(depth_dec):
            dec_attns = nn.ModuleList([AttnBlock(latent_dim, num_heads) for _ in range(self_per_cross_attn)])
            self.dec_block.append(nn.ModuleList([CrossAttnBlock(latent_dim, cross_heads), dec_attns]))
    ...
    def forward(
        ...
        for block in self.enc_block:
            cross_attn, attns = block[0], block[-1]
            x = torch.utils.checkpoint.checkpoint(self.ckpt_wrapper(cross_attn), x, input_emb, c)
            for l in range(len(attns)):
                x = torch.utils.checkpoint.checkpoint(self.ckpt_wrapper(attns[l]), x, c)
                
        for block in self.dec_block:
            cross_attn, attns = block[0], block[-1]
            queries = torch.utils.checkpoint.checkpoint(self.ckpt_wrapper(cross_attn), queries, x, c)
            for l in range(len(attns)):
                queries = torch.utils.checkpoint.checkpoint(self.ckpt_wrapper(attns[l]), queries, c)
